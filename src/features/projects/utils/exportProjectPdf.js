...
export function exportProjectPdf(project, historyItems) {
  ...
  <div class="box"><h2>שעות עבודה</h2><table><thead><tr><th>התחלה</th><th>סיום</th><th>צוות נוסף</th><th>הערה</th><th>מיקום התחלה</th><th>מיקום סיום</th></tr></thead><tbody>${sessions.length ? sessions.map((w) => `<tr><td>${new Date(w.started_at).toLocaleString("he-IL")}</td><td>${w.ended_at ? new Date(w.ended_at).toLocaleString("he-IL") : "פתוח"}</td><td>${w.crew_members?.length ? w.crew_members.map((member) => escapeHtml(member.name)).join(', ') : '-'}</td><td>${escapeHtml(w.end_note || '-')}</td><td>${mapsLink(w.started_lat, w.started_lng) ? `<a href="${mapsLink(w.started_lat, w.started_lng)}">מפה</a>` : "-"}</td><td>${mapsLink(w.ended_lat, w.ended_lng) ? `<a href="${mapsLink(w.ended_lat, w.ended_lng)}">מפה</a>` : "-"}</td></tr>`).join("") : '<tr><td colspan="6">אין שעות עבודה</td></tr>'}</tbody></table></div>
