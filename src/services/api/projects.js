...
export async function getProjectAssets(projectId) {
  const [photos, reviewFiles, documents] = await Promise.all([
    ...
    supabase.from('project_documents').select('id,project_id,uploaded_by,file_path,file_name,file_size,mime_type,document_type,created_at,profiles:uploaded_by(full_name)').eq('project_id', projectId).order('created_at', { ascending: false }),
  ]);
