...
-- Keep this migration safe when the base attendance table was already created.
...
-- Prevent accidental duplicate open shifts, including concurrent button presses.
create unique index if not exists attendance_sessions_one_open_per_worker_idx
  on public.attendance_sessions(worker_id)
  where ended_at is null;

-- Only one all-day absence/status report per employee and date.
